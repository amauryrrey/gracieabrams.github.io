const songsData = [
  ...
  // Daughter from Hell
  { name: "Hit_the_Wall", album: "Daughter_from_Hell" },
  { name: "Humming", album: "Daughter_from_Hell" },
  { name: "Minibar", album: "Daughter_from_Hell" },
  { name: "Daughter_from_Hell", album: "Daughter_from_Hell" },
  { name: "Good_Reason", album: "Daughter_from_Hell" },
  { name: "Imaginary_Friend", album: "Daughter_from_Hell" },
  { name: "What_If_Its_Right", album: "Daughter_from_Hell" },
  { name: "Mews", album: "Daughter_from_Hell" },
  { name: "Men_Like_You", album: "Daughter_from_Hell" },
  { name: "Broke_My_Heart", album: "Daughter_from_Hell" },
  { name: "Cold_Goodbyes", album: "Daughter_from_Hell" },
  { name: "The_Knife", album: "Daughter_from_Hell" },
  { name: "Look_at_My_Life", album: "Daughter_from_Hell" },
  { name: "Sober", album: "Daughter_from_Hell" },
  { name: "Afflictions", album: "Daughter_from_Hell" },
  { name: "Death_Wish", album: "Daughter_from_Hell" }
];
let songs = [
      "Mean_It",
      "Stay",
    ...
    let songFiles = [
      ...
      "61_Humming.mp3",
      "62_Minibar.mp3",
      "63_Daughter_from_Hell.mp3",
      "64_Good_Reason.mp3",
      "65_Imaginary_Friend.mp3",
      "66_What_If_Its_Right.mp3",
      "67_Mews.mp3",
      "68_Men_Like_You.mp3",
      "69_Broke_My_Heart.mp3",
      "70_Cold_Goodbyes.mp3",
      "71_The_Knife.mp3",
      "72_Look_at_My_Life.mp3",
      "73_Sober.mp3",
      "74_Afflictions.mp3",
      "75_Death_Wish.mp3"
    ];
...
// Guardamos una copia maestra de las canciones para poder filtrarlas sin perder datos
const masterSongs = [...songs];
const masterSongFiles = [...songFiles];
...
// Al darle click a Start, ahora abrimos el modal en vez de empezar directo
startGameBtn.addEventListener("click", () => {
  modeModal.style.display = "block";
});

// Para cerrar el modal con la tachita
closeModalBtn.addEventListener("click", () => {
  modeModal.style.display = "none";
});

// Función que filtra las canciones según el botón que presiones
function selectMode(mode) {
  modeModal.style.display = "none"; // Cerramos el modal

  // Obtenemos los puros nombres de las canciones que SÍ son de DFH
  const dfhNames = songsData
    .filter(s => s.album === "Daughter_from_Hell")
    .map(s => s.name);

  if (mode === 'all') {
    // Si elige todas, reiniciamos las listas a la versión maestra
    songs = [...masterSongs];
    songFiles = [...masterSongFiles];
  } 
  else if (mode === 'dfh') {
    // Solo dejamos las que estén en el arreglo de DFH
    songs = masterSongs.filter(name => dfhNames.includes(name));
    songFiles = masterSongFiles.filter(file => {
      const cleanName = file.slice(file.indexOf("_") + 1, file.lastIndexOf("."));
      return dfhNames.includes(cleanName);
    });
  } 
  else if (mode === 'no_dfh') {
    // Solo dejamos las que NO estén en el arreglo de DFH
    songs = masterSongs.filter(name => !dfhNames.includes(name));
    songFiles = masterSongFiles.filter(file => {
      const cleanName = file.slice(file.indexOf("_") + 1, file.lastIndexOf("."));
      return !dfhNames.includes(cleanName);
  ...
  // Ahora sí, con la lista filtrada, arrancamos el juego
  startGame();
}
